class Controller {
  ...
  filterRecipes(selectedIngredients, selectedAppareils, selectedUstensiles) {
    ...
    const filteredRecipes = this.filterByIngredients(
      allRecipes,
      selectedIngredients
    )
  ...
  searchByText(filteredRecipes, searchValue) {
    let results = [];
    console.log("la valeur cherché est :", searchValue);
    for (let i = 0; i < filteredRecipes.length; i++) {
      ...
      if (recipe.name.toLowerCase().includes(searchValue)) {
        ...
        results.push(recipe);
      } else if (recipe.appliance.toLowerCase().includes(searchValue)) {
        results.push(recipe);
      } else if (this.arrayToLowerCase(recipe).includes(searchValue)) {
        results.push(recipe);
      } else {
        let ingredientsIncluded = false;
        for (let j = 0; j < recipe.ingredients.length; j++) {
          if (
            recipe.ingredients[j].ingredient.toLowerCase().includes(searchValue)
          ) {
  ...
  arrayToLowerCase(recipe) {
    for (let j = 0; j < recipe.ustensils.length; j++) {
      recipe.ustensils[j] = recipe.ustensils[j].toLowerCase();
